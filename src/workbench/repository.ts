...
const WORKBENCH_SCHEMA_STATEMENTS = [
  ...
  `INSERT INTO workbench_meta(key, value) VALUES('schema_version', '1') ON CONFLICT(key) DO NOTHING`,
] as const;
...
async function ensureWorkbenchSchema(db: Database): Promise<void> {
  for (const statement of WORKBENCH_SCHEMA_STATEMENTS) {
...
async function database(): Promise<Database | null> {
  ...
  databasePromise ??= import("@tauri-apps/plugin-sql").then(async ({ default: Database }) => {
    ...
    await ensureWorkbenchSchema(db);
    return db;
  });
