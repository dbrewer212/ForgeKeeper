...
import type { PrinterRecord, PrinterStatus } from "../../types/domain";

const statuses: PrinterStatus[] = ["Available", "Printing", "Maintenance", "Offline"];
type ExtendedPrinter = PrinterRecord & { controlUrl?: string };

function printerFamily(printer: PrinterRecord): "anycubic" | "fluidd" | "other" {
  const identity = `${printer.name} ${printer.model}`.toLowerCase();
  if (identity.includes("kobra") || identity.includes("anycubic")) return "anycubic";
  if (identity.includes("neptune") || identity.includes("elegoo")) return "fluidd";
  return "other";
}
    ...
    <div className="space-y-6">
      ...
      <Card title="Printers" right={<Button variant="ghost" onClick={state.exportPrintersCsv}>Export CSV</Button>}>
        <div className="grid gap-4 xl:grid-cols-2">
          ...
          ) : state.printers.map((printerBase) => {
            const printer = printerBase as ExtendedPrinter;
            const family = printerFamily(printer);
            const controlLabel = family === "fluidd" ? "Fluidd" : family === "anycubic" ? "Anycubic Slicer Next" : "Web Console";
            return (
              <div key={printer.id} className="rounded-2xl border border-white/10 bg-[#0d131c] p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-xs text-slate-500">Control route: {controlLabel}</div>
                  </div>
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  ...
                  <Input
                    value={printer.controlUrl ?? ""}
                    onChange={(e) => state.updatePrinter(printer.id, { controlUrl: e.target.value } as Partial<PrinterRecord>)}
                    placeholder={family === "fluidd" ? "Fluidd URL, example: http://192.168.1.42" : "Optional printer web-console URL"}
                  />
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {printer.controlUrl ? (
                    <Button variant="ghost" onClick={() => void openUrl(printer.controlUrl || "", `${printer.name} control`)}>Open {family === "fluidd" ? "Fluidd" : "Web Control"}</Button>
                  ) : null}
                  {family === "anycubic" && !mobile ? (
                    <Button variant="ghost" onClick={() => void launchExternalTool(state.settings.anycubicSlicerPath || "", undefined, "Anycubic Slicer Next")}>Open Anycubic Control</Button>
                  ) : null}
                ...
                {family === "fluidd" && !printer.controlUrl ? (
                  <div className="mt-3 rounded-xl border border-amber-800/30 bg-amber-950/15 p-3 text-xs leading-5 text-amber-200/80">Add this printer's Fluidd URL above to make its native control surface directly reachable from Forgekeeper.</div>
                ) : null}
