...
export function FilamentView({ state }: { state: ForgekeeperState }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card title="Filament Inventory Census" right={<span className="text-xs text-slate-500">v0.1.4 · {state.storageStatus === "SQLite" ? "SQLite workshop database" : state.storageStatus}</span>}>
        {state.storageStatus === "Error" && (
          <div className="mb-4 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-200">
            <div className="font-semibold">SQLite workshop database did not start.</div>
            <div className="mt-1 break-all text-xs text-red-100/80">{state.storageError || "No diagnostic detail was returned."}</div>
          </div>
        )}
