...
const checks = [
  ...
  ["pending-workspace-retries-before-local-publish", files.desktopRuntime.includes("getStagedDesktopPendingWorkspace()") && files.desktopRuntime.includes("if (await servicePendingWorkspace()) return") && files.desktopRuntime.indexOf("if (await servicePendingWorkspace()) return") < files.desktopRuntime.indexOf("publishCurrentWorkspace(")],
