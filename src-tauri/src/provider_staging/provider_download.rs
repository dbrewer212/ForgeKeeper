...
pub(super) async fn download_provider_asset_safely(
    ...
    let download_url = match provider.as_str() {
        "meshy" => {
            let key = credentials.meshy.ok_or(
                "No Meshy key was found in the credential file or MESHY_API_KEY environment variable.",
            )?;
            let response = client
...
fn read_provider_credentials(api_file_path: &str) -> Result<ProviderCredentials, String> {
    ...
    if !api_file_path.trim().is_empty() {
        ...
        for raw_line in contents.lines() {
            ...
            if value.starts_with("pp_live_")
                || label.as_deref().is_some_and(|item| item.contains("printpal"))
            {
                credentials.printpal = Some(value.to_string());
            } else if label.as_deref().is_some_and(|item| item.contains("meshy"))
                || credentials.meshy.is_none()
            {
                credentials.meshy = Some(value.to_string());
            }
        }
...
fn validate_target_extension(path: &Path, format: &str) -> Result<(), String> {
    ...
    if extension != format.to_ascii_lowercase() {
        let shown = if extension.is_empty() { "unknown" } else { &extension };
        return Err(format!(
            "Provider staging refused extension mismatch: requested .{format}, target path is .{shown}."
        ));
    }
    Ok(())
}
